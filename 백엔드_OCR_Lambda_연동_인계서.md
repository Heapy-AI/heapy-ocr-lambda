...
- 작성자: 김진우
- 작성일: 2026-09-07
- 상태: 2026-09-08 합성 OCR 통합 검증 및 최초 GitHub dev 자동 배포 성공. Worker·Janitor live 버전 2. 앱 연동·한국어 양식 품질 검증은 남아 있음.
...
Worker와 Janitor 모두 live 버전 2 전환 후 스모크 통과. 기존 함수·버킷·별칭 ARN을 그대로 사용한다.
개발 자동 배포 승인 변수는 true이며 운영 승인은 미설정이다. 아래 최초 이미지·버전 1·미활성 기록은
이전 이력이다. 새 버전의 실제 문서 OCR과 실환경 롤백은 이번 스모크로 검증한 것이 아니다.
...
| 20MB | 사용자가 바이트 기준 확정 | 개발 `MAX_UPLOAD_BYTES=20000000` |
| 결과 TTL | 사용자가 10분으로 확정 | 개발 `MAX_RESULT_TTL_SECONDS=600`, 작업 생성부터 계산하며 처리 시간 포함 |
| Gemini 모델 | 사용자 지정 Flash-Lite 2.5 | 개발 `GEMINI_MODEL=gemini-2.5-flash-lite`, 실제 인식 품질은 아직 미검증 |
...
최신 확정 정책: 원본은 OCR 결과 생성 직후 삭제하고 삭제 성공 전 완료 결과를 공개하지 않는다.
실패·취소도 원본을 삭제하며 강제 종료·삭제 장애는 회수기가 재시도한다. 600초는 검수 결과와
작업 유효기간의 상한이며 원본을 600초 보관하라는 의미가 아니다. 백엔드는 createdAt부터
expiresAt까지 600초 이내로 생성해야 하고, 처리 시간이 길면 남은 검수 시간은 그만큼 짧아진다.
600초 경계에서 read를 차단하며 물리 삭제 장애가 조회 허용으로 이어지지 않는다.
