...
- 실제 AWS 상태: 합성 PDF OCR·결과 제거·정리 후 원본 부재 확인 완료. GitHub dev 자동 배포 활성화 및 최초 배포 성공(Worker·Janitor live 버전 2). 한국어 인식 품질·앱 연동 검증은 남아 있음.
...
일반건강검진 결과통보서와 기관별 종합검진 보고서의 공통 검사 결과 표를 대상으로 합니다.
종합검진 문서라는 이유만으로 제외하지 않으며, 현재 검사와 과거 비교·그래프·상세 소견을
영역별로 구분합니다. 종합검진 전체 내용의 저장을 지원하는 것은 아닙니다. 키·몸무게, 혈압과 시력의
...
현재 열이 불명확한 비교표도 추정하지 않습니다. 구분자가 있는 표에서는 명시적인 금회 결과
열을 선택하며, 구분자가 사라진 과거 비교표는 보수적으로 제외합니다.
실제 외부 API 인식 정확도는 아직 검증하지 않았으며 사용자 검수가 필요합니다.

## 파일과 보존

JPG/JPEG/PNG/PDF, 최대 20,000,000바이트·PDF 20페이지를 검증합니다. 개발 환경의
최대 TTL은 작업 생성부터 600초이며 처리 시간도 포함합니다. 모델은 `gemini-2.5-flash-lite`입니다.
이 값들은 런타임 생성 매개변수로 전달합니다. 비밀번호 없는 PDF만 지원하며
...
- [다양한 검진 양식 지원 인계](OCR_다양한_검진양식_지원_인계.md): 현재 결과 추출, 검증 한계, DB·화면 협업
