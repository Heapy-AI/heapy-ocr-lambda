...
사용자가 제공한 버전 3 로그에서 2026-09-08 12:38:40 및 12:42:31 한국시간 호출 모두
provider=gemini, callIndex=1, code=TIMEOUT, httpStatus=None을 확인했다.
durationMs는 각각 60224, 60227이다. 시간대 표시 문제가 아니라 새 실행에서 실제 시간 초과가 발생했다.
응답을 받지 못한 이유가 외부 서버 지연인지 네트워크 문제인지는 이 로그만으로 단정하지 않는다.

우선 건강검진 Gemini 요청의 기본 묶음을 3페이지에서 1페이지로 줄인다. 60초 제한은 유지해
한 요청이 전체 600초 예산을 과도하게 점유하지 않게 한다. 복약의 페이지 처리 방식은 변경하지 않는다.
API 호출 횟수는 늘 수 있으며, 20페이지를 모두 600초 안에 완료한다고 보장하지 않는다.
GEMINI_PAGES_PER_REQUEST 환경변수가 별도 설정돼 있다면 환경값이 우선한다. 현재 인프라
템플릿에는 이 변수가 없어 코드 기본값을 사용한다. 배포 후 같은 PDF를 새 작업으로 한 번 확인한다.

배포 완료: [Actions 34183895003](https://github.com/Heapy-AI/heapy-ocr-lambda/actions/runs/34183895003)
성공. 커밋 `06d291cb4cd24a810271677bed4551a3ed8226b0`, Worker·Janitor live 버전 `3`.
이미지 digest는 `sha256:16b2ded56de0b2228a3505bd943062dfa45bf9093b63cabc0d5566070d99013b`다.
테스트 85개·Linux 빌드·배포 후 스모크 통과. 동일 PDF의 앱 재업로드·검수 저장은 사용자 실행 대기다.
